...
**没耐心读 Claude 的长篇大论？** duckit 是一个 Claude Code 扩展（叫 "skill"，就是丢进 `~/.claude/skills/` 自动激活的小插件），敲 `/duckit` 把上一条 assistant 消息压成中文鸭子语 — 代码、命令、路径、数字 **byte-exact 不变**，你照样复制粘贴跑。
...
复制粘贴 → 回车 → 完事。打开 Claude Code，任何对话里敲 `/duckit` — 上一条回答瞬间变鸭子版。
